...
class IpfsUrlResolver(
    ...
    private val innerGateway = ipfsProperties.ipfsGateway
        .split(",")
        .map { it.trimEnd('/') }

    private val publicGateway = ipfsProperties.ipfsPublicGateway.trimEnd('/')

    private val legacyGateway = ipfsProperties.ipfsLegacyGateway
        ?.split(",")
        ?.map { it.trimEnd('/') } ?: emptyList()
