...
@Component
class EnrichedOwnershipApiService(
    private val orderApiService: OrderApiService,
    private val ownershipQueryService: OwnershipQueryService,
    ...
    suspend fun getOwnershipByOwner(
        ...
        val ownerships = ownershipQueryService
            .getOwnershipByOwner(owner, continuation, size)

        // owner auctions
        val ownerAuctions = enrichmentAuctionService.findBySeller(owner).mapAsync {
    ...
    suspend fun getOwnershipsByItem(itemId: ItemIdDto, continuation: String?, size: Int): OwnershipsDto {
        ...
        val itemAuctions =
            enrichmentAuctionService.findByItem(shortItemId).mapAsync {
                val ownershipId = it.getSellerOwnershipId()
        ...
        val ownerships = ownershipQueryService
            .getOwnershipsByItem(itemId, continuation, totalSize)
            .filter {
        ...
        return OwnershipsDto(ownerships.size.toLong(), page.continuation, result)
    }
    ...
    private suspend fun merge(combined: List<UnionAuctionOwnershipWrapper>): List<UnionOwnership> =
