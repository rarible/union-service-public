...
class EthActivityServiceTest {
    ...
    fun `should get activities by item and owner`() = runBlocking<Unit> {
        ...
        val unionMint = mockk<UnionMintActivity> {
            every { id } returns ActivityIdDto(blockchainDto, "A")
            every { date } returns dateMint
        }
        val unionTransfer = mockk<UnionMintActivity> {
    ...
    fun `should get activities by ids - happy path`() = runBlocking<Unit> {
        ...
        every {
            activityOrderControllerApi.getOrderActivitiesById(
        ...
        every {
            activityAuctionControllerApi.getAuctionActivitiesById(
                ActivitiesByIdRequestDto(
                    listOf(
                        "ETHEREUM:C",
                        "ETHEREUM:F"
