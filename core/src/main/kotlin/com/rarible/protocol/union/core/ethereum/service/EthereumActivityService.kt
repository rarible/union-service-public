...
class EthereumActivityService(
    ...
    override suspend fun getAllActivities(
        ...
        return getEthereumActivities(filter, continuation, size)
    }
    ...
    override suspend fun getActivitiesByUser(
        ...
        val filter = ActivityFilterByUserDto(
            ...
            LinkedHashSet(types).map { EthUnionActivityConverter.convert(it) }
        )
        return getEthereumActivities(filter, continuation, size)
    }

    private suspend fun getEthereumActivities(filter: ActivityFilterDto, continuation: String?, size: Int?) =
        coroutineScope {
            val itemRequest = async { getItemActivities(filter, continuation, size) }
            val orderRequest = async { getOrderActivities(filter, continuation, size) }
    ...
    private suspend fun getItemActivities(
        ...
        return if (itemFilter != null) {
            activityItemControllerApi.getNftActivities(itemFilter, continuation, size).awaitFirst()
        } else {
    ...
    private suspend fun getOrderActivities(
        ...
        return if (orderFilterDto != null) {
            activityOrderControllerApi.getOrderActivities(orderFilterDto, continuation, size).awaitFirst()
        } else {
